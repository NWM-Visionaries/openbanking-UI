...
export default function AccountObject ({account})
{
    ...
    async function fetchAccountRecommendations(accountId)
    {
        ...
        console.log('http://localhost:4002/open-banking/recommendation/getRecs?accountId='+accountId)
      const res = await fetch('https://cors-anywhere.herokuapp.com/http://localhost:4002/open-banking/recommendation/getRecs?accountId='+accountId);
      ...
      const responseJSON = res.data
        ...
        .then(data => {console.log(data)
            ...
            for (let index = 0; index < data.Actions.length; index++) {
                recomdString = recomdString+ " Rec " + (index+1) + " :- "+ data.Actions[index].Action
            }
    ...
    return (
        <>
            ...
            <td><button onClick={() => fetchAccountRecommendations(account.AccountId)}>Get Recommendation</button></td>
            </tr>
        </>
    )
}   